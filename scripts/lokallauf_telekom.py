"""Der woechentliche Telekom-Lokallauf (T1: Tarife, T2: Geraete) - mit
Laufzeitbeleg fuer T1.
...
`config/tarif_quellen.yaml` traegt seit BRIEF_TELEKOM_TAEGLICH_R3_E4 zudem
einen Per-Anbieter-Override: NUR Telekom bekommt den ehrlichen
`TelcoRadar/1.0`-Absender, `config/settings.yaml -> http.user_agent` (die
globale Chrome-Kennung fuer alle anderen Quellen) bleibt unangetastet. Der
Absender ist also nicht (mehr) die Ursache der 202-Challenge - eine gemessene
`TelcoRadar/1.0`-Anfrage kam am 06.09.2026 direkt gegen telekom.de mit HTTP
200 durch (`outputs/telekom-taeglich-r3-e4-2026-09-06.md`). H1 (Lokallauf von
Hand statt Actions) bleibt trotzdem in Kraft: der Unterschied liegt am
IP-Bereich, nicht am Absender.

WAS ES TUT
----------
1. `tarif_crawler.sammle()` NUR fuer die Telekom-Tarifquellen (T1: das
   Pflichtdokument-Verzeichnis UND die Shop-Tarifkacheln,
   `methode: telekom_kacheln`) - mit einer Huellfunktion um
   `collect.http.fetch`, die JEDEN echten Abruf protokolliert: URL, Host,
   HTTP-Status, der tatsaechlich GESENDETE `User-Agent` (aus
   ...
   Playwright/Selenium/Headless-Browser). Keine Antwortkoerper, keine
   Cookies, keine Zugangsdaten - nur die Kopfzeile, die den Absender
   verraet.

   Der Lauf schreibt den Beleg nach
   `outputs/beleg-telekom-lokallauf-<datum>.json` (versioniert im Repo,
   nicht unter `data/state/` - das wird nie committet) und prueft am Ende
   SELBST, ob ALLE Eintraege mit der ehrlichen Kennung gesendet wurden
   ...
   stillschweigend als Erfolg meldet - ein Befund wird als `ERROR`
   protokolliert, nicht verschwiegen.

   `fetch()` selbst bleibt unveraendert; die Instrumentierung ist
