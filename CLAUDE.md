...
| `data/state/geraete_tco_historie.jsonl` | Die Buendel-Preishistorie (P2, 11.09.2026): eine Zeile je (buendel_id, datum) mit den Messfeldern, der eingefrorenen Leitzahl `gesamt` und `abgerufen_am`. Gleicher Messtag ERSETZT seine Zeile (idempotent), ein neuer Tag haengt an, alte Tage bleiben unveraendert; ehrlich ab dem ersten Lauf begonnen - nichts aus alten Stand-Commits zurueckgerechnet. `geraete_tco.json` bleibt der aktuelle Stand, die Seite liest nur ihn |
